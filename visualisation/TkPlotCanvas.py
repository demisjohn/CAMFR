    ...
    def writeToFile(self, file, separator):
        if self.points.any():
            for p in self.points:
                file.write(str(p[0]) + separator + str(p[1]) + '\n')
            return 1
        else:
            return 0
...
class PlotCanvas(Frame):
    ...
    def _mouseMotion(self, event):
        if self.mouse_state == 0:
            ...
            p = (numpy.array([self.startx, self.starty])-shift)/scale
            bb1, bb2 = self.bbox
    ...
    def _mouseRelease(self, event):
        if self.mouse_state == 1:
            self.canvas.delete(self.rubberband)
            self.rubberband = None
            p1 = numpy.array([self.startx, self.starty])
            p2 = numpy.array([self.canvas.canvasx(event.x),
                                self.canvas.canvasy(event.y)])
            if numpy.minimum.reduce(numpy.fabs(p1-p2)) > 5:
                scale, shift = self.transformation
                p1 = (p1-shift)/scale
                p2 = (p2-shift)/scale
