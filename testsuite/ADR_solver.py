...
class ADR_solver(unittest.TestCase):
    def testADRsolver(self):
        ...
        FeCo         = Material(3.565458-7.13499488050650*1j)
        InP          = Material(3.18)
        InGaAs       = Material(3.6-0.0806676*1j)
        ...
        n_eff_9    = s_ADR.mode(9).n_eff()        
        n_eff_9_OK = 1.48931-0.0346762j

        print n_eff_9, "expected", n_eff_9_OK
        mode9_pass = abs((n_eff_9 - n_eff_9_OK)/n_eff_9_OK) < eps.testing_eps
        
        free_tmps()
        set_solver(track)

        self.failUnless(mode0_pass and mode9_pass)
